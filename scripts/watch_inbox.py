...
def process_inbox(receiver):
    ...
    passphrase = getpass.getpass(f"[🔐] Enter passphrase for {receiver}'s private key: ")
        ...
        for meta_file in files:
            ...
            try:
                ...
                if verify_signature(decrypted_data, signature, sender_pub):
                    ...
                    print(f"[✓] Message from {sender} verified and saved: {out_path.name}")
                else:
                    print(f"[✗] Signature verification failed for {prefix}")
            except Exception as e:
                print(f"[✗] Failed to process {prefix}: {str(e)}")
